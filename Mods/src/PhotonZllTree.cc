...
ClassImp(mithep::PhotonZllTree)

void
mithep::PhotonZllTree::Process()
{
  ...
  for (unsigned iM = 0; iM != sizeof(outMets) / sizeof(photonzll::Met*); ++iM) {
  ...
  if (fFillZmumu) {
    ...
    if (muons->GetEntries() != 2) {
      SendError(kAbortAnalysis, "Process", "!= 2 muons");
      return;
    }
    
    auto& mu0(*muons->At(0));
    fEvent.z.pt0 = mu0.Pt();
    fEvent.z.eta0 = mu0.Eta();
    fEvent.z.phi0 = mu0.Phi();
    auto& mu1(*muons->At(1));
    fEvent.z.pt1 = mu1.Pt();
    fEvent.z.eta1 = mu1.Eta();
    fEvent.z.phi1 = mu1.Phi();
  }
  ...
  for (unsigned iP = 0; iP != photons->GetEntries(); ++iP) {
    ...
    auto& outPhoton(fEvent.photons[iP]);
...
void
mithep::PhotonZllTree::SlaveBegin()
{
